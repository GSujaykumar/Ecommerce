...
import { fetchProductsByCategory, fetchMensProducts, fetchWomensProducts, fetchKidsProducts, fetchHomeProducts, fetchElectronicsProducts, fetchSubCategoriesByCategory } from "../api";
import { useParams } from "react-router-dom";
import { motion, AnimatePresence } from "framer-motion";
import { FiFilter, FiX } from "react-icons/fi";

function CategoryPage({ categoryName: propCategoryName, apiCategory: propApiCategory, type: propType }) {
    const { categoryName: urlCategoryName } = useParams();
    const activeCategory = urlCategoryName || propType || propApiCategory || propCategoryName;
    ...
    useEffect(() => {
        const getProducts = async () => {
            ...
            // Directly fetch by category from API for all types
            const data = await fetchProductsByCategory(activeCategory.toLowerCase());

            setAllProducts(data);
            setFilteredProducts(data);

            // Fetch dynamic sub-categories
            const subs = await fetchSubCategoriesByCategory(activeCategory.toLowerCase());
                                        ...
                                        {subCategories.map((sub) => (
                                            <label key={sub} className="flex items-center gap-3 cursor-pointer group">
                                                <div className="relative flex items-center">
                                                    <input
                                                        type="checkbox"
                                                        className="peer w-5 h-5 rounded border-2 border-gray-300 dark:border-gray-700 text-indigo-600 focus:ring-indigo-500 transition-all cursor-pointer checked:bg-indigo-600 checked:border-indigo-600"
                                                        onChange={(e) => {
                                                            if (e.target.checked) setFilteredProducts(allProducts.filter(p => p.subCategory?.toLowerCase() === sub.toLowerCase()));
                                                            else setFilteredProducts(allProducts);
                                                        }}
                                                    />
                                                </div>
                                                <span className="text-sm font-medium text-gray-700 dark:text-gray-300 capitalize group-hover:text-indigo-600 transition-colors">{sub}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
