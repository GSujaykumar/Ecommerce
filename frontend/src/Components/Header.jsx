...
function Header() {
    ...
    const linkClass = ({ isActive }) =>
        `text-sm font-medium transition duration-200 capitalize ${isActive
            ...
            <nav className="mx-auto flex max-w-7xl items-center justify-between p-4 lg:px-8">
                ...
                <div className="hidden lg:flex lg:gap-x-10">
                    ...
                    {dynamicCategories.map(cat => (
                        <NavLink key={cat} to={`/category/${cat}`} className={linkClass}>{cat}</NavLink>
                    ))}
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                {dynamicCategories.map((cat) => (
                                    <NavLink
                                        ...
                                        to={`/category/${cat}`}
                                        onClick={() => setIsMobileMenuOpen(false)}
                                        className={({ isActive }) => `text-lg font-medium capitalize ${isActive ? 'text-indigo-600' : 'text-gray-900 dark:text-gray-100'}`}
                                    >
                                        {cat}
                                    </NavLink>
                                ))}
